"""Tests for main.py entry points — covering start() and main()."""
...
class TestMainFunction:
    """Cover main.main() which calls cli.run(cli.parse_args()) then sys.exit."""
...
class TestStartFunction:
    """Cover main.start() — bootstrap script entry point."""
    ...
    def test_start_bootstrap_not_found(self, tmp_path):
        ...
        fake_file = tmp_path / "src" / "simple_python_boilerplate" / "main.py"
        fake_file.parent.mkdir(parents=True)
        fake_file.touch()

        with (
            patch("simple_python_boilerplate.main.__file__", str(fake_file)),
    ...
    def test_start_runs_bootstrap(self, tmp_path):
        ...
        fake_file = fake_root / "src" / "simple_python_boilerplate" / "main.py"
        fake_file.parent.mkdir(parents=True)
        fake_file.touch()
