...
**Command reference (point-in-time snapshot):**

```bash
task docs:commands             # Regenerate docs/reference/commands.md
task docs:commands:check       # Exit 1 if the command reference is stale
task docs:commands -- --dry-run # Preview output without writing to disk
```

!!! important "Command reference is a snapshot, not live"
    The command reference page (`docs/reference/commands.md`) is a
    **point-in-time snapshot** generated by
    `scripts/generate_command_reference.py`. It is **not** regenerated
    automatically during `mkdocs build` or `mkdocs serve` — you must run
    `task docs:commands` manually whenever tasks or scripts change.

    To keep it fresh before building/deploying docs:

    ```bash
    task docs:commands       # Regenerate the command reference
    task docs:build          # Then build the docs site
    ```

    Use `task docs:commands:check` in CI to catch forgotten regenerations.
