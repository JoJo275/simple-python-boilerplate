...
<!-- Generated: 2026-03-26 19:42:29 UTC -->
...
!!! info "Generation details"
    ...
    | **Timestamp** | 2026-03-26 19:42:29 UTC |
...
??? note "Full `--help` output"
    ...
    usage: env-inspect [-h] [--version] [--json]
                       [--section {python,git,venv,packages,entrypoints,build-tools,python-support,path}]
                       [--quiet] [--no-color]
...
??? note "Full `--help` output"
    ...
    options:
      ...
      --dry-run             With --apply-from, --apply-recommended, --apply-
                            recommended-minimal, --refresh, or --cleanup: show
                            what would be changed without applying
...
??? note "Full `--help` output"
    ...
    ```text
    usage: repo-sauron [-h] [--version] [--json]
                       [--section {overview,files,languages,directories,largest,git,contributors}]
                       [--quiet] [--no-color]

    Repository statistics dashboard � the all-seeing eye.
