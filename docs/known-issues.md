...
| **Security** | PGP key import path unclear      | `SECURITY.md` GPG import instructions didn't clarify that `gpg --import pgp-key.asc` requires running from the repo root or using a full path. Fixed — docs now explain both options. | Low      |
